...
pub fn knn_recommend(k: i32, idx: String, distance_type: Distance) {
    let controller = BookController::new();
    
    let mut result_user = Vec::new();

    result_user = controller.get_user_by_id(idx.parse().expect("Error parsing userid"));
    ...
    for user in &result_user {
        ...
        println!("Book database, recommendations for user ({})", user.id);
        let mut i = 0;
        for it in &result {
            if i < 5 {
                let item = &controller.get_item_by_id(it.id.clone())[0];
                println!("{} Item {}({}) with score: {}", i+1, item.title, item.id, it.distance);
                i += 1;
            } else {
                println!();
                return;
